...
# Custom CSS for modern and clean UI
st.markdown("""
    <style>
    .stApp { background-color: #f9fafb; font-family: 'Arial', sans-serif; }
    .big-font { font-size: 30px !important; font-weight: bold; color: #4a90e2; text-align: center; margin-bottom: 20px; }
    .subtitle { text-align: center; font-size: 18px; color: #4b5563; margin-bottom: 30px; }
    .csv-download-button { display: inline-block; padding: 10px 20px; background-color: #4a90e2; color: white !important; border-radius: 8px; font-weight: 500; text-align: center; transition: background-color 0.3s ease; width: 100%; }
    .csv-download-button:hover { background-color: #357abd; }
    .stButton > button { background-color: #4a90e2; color: white; border-radius: 8px; padding: 10px 20px; font-weight: 500; transition: background-color 0.3s ease; width: 100%; }
    .stButton > button:hover { background-color: #357abd; }
    .stTextInput > div > div > input, .stSelectbox > div > div > select { border-radius: 8px; border: 1px solid #d1d5db; padding: 10px; }
    .stExpander { border: 1px solid #e5e7eb; border-radius: 8px; background-color: #ffffff; }
    .footer { text-align: center; color: #6b7280; font-size: 14px; margin-top: 40px; padding: 20px 0; border-top: 1px solid #e5e7eb; }
    </style>
...
def create_multi_level_headers(input_dataframe):
    """Create properly structured multi-level headers."""
    if len(input_dataframe.columns) == TABLE_CONFIG['expected_columns']:
        ...
        return processed_dataframe.reset_index(drop=True), True
    return input_dataframe, False

def show_table_structure(processed_dataframe):
    """Display the column structure and a preview of rows."""
    with st.expander("🔍 Table Structure & Preview"):
        ...
        if isinstance(processed_dataframe.columns, pd.MultiIndex):
            for j, (year, measure) in enumerate(processed_dataframe.columns):
                year_clean = year.split('(')[0].strip() if str(year).strip() else ""
                st.write(f"Column {j+1}: **{year_clean}** - {measure}" if year_clean else f"Column {j+1}: {measure}")
        else:
        ...
        if len(processed_dataframe) > 0:
            ...
            st.warning("⚠️ No rows extracted.")
...
def download_buttons(input_dataframe, table_index, session_id, has_multi_level_headers):
    ...
    with col_excel:
        ...
        if has_multi_level_headers and isinstance(input_dataframe.columns, pd.MultiIndex):
            excel_df.columns = [' - '.join([str(l) for l in col if l]) for col in input_dataframe.columns]
...
if uploaded_file:
    ...
    with col2:
        extraction_mode = st.selectbox("Extraction Mode", ["Lattice (borders/grid)", "Stream (aligned text)"])
    with col3:
        output_format = st.selectbox("Output Format", ["CSV (multi-row headers)", "Excel (multi-level headers)", "Both"])
            ...
            try:
                ...
                with st.spinner("Extracting tables..."):
                    ...
                    if not tables or all(t.empty for t in tables):
                        st.warning("⚠️ No tables extracted with Tabula. Trying OCR...")
        ...
        if len(st.session_state.tables) > 0:
            ...
            for i, table in enumerate(st.session_state.tables):
                ...
                if has_multi_level_headers:
                    ...
                    st.markdown("**Standard table detected**")
                    st.dataframe(processed_dataframe, use_container_width=True)
                    download_buttons(processed_dataframe, i, st.session_state.session_id, False)

# Footer
st.markdown("""<div class="footer"><p>Developed by dev.fero | Bharatpur-11, Chitwan</p><p>© 2025 FeroCS Solutions</p></div>""", unsafe_allow_html=True)
